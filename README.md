...
部署后扫码登录一次，再通过外部定时任务每 6 到 12 小时调用保活接口：

```bash
curl -fsS \
  -H "X-Auth-Key: $NUXT_MP_STABLE_AUTH_KEY" \
  https://你的域名/api/web/mp/keepalive
```

每次成功访问微信公众号后台都会合并微信最新下发的 Cookie，并重新计算 KV 和浏览器 Cookie 的有效期。固定 API Key 只适用于可信的私有单账号部署，请不要暴露给他人。

该机制可以去掉项目原有的 4 天硬过期并持续续期，但无法绕过微信服务端主动撤销登录、改密、风控等情况；微信判定会话失效后仍需重新扫码。
